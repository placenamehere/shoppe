...
sjs.doc = {
  newIfNone: function(w,h,dpi) {
    ...
    if (app.documents.length > 0) {
      ...
      d = app.documents.add(w,h,dpi,null,NewDocumentMode.RGB, DocumentFill.WHITE);
  ...
  begin: function(d) {
    d = d || sjs.doc.newIfNone();
    sjs.doc.snapshot(d);
    sjs.modeReset.modeReset(d);
  }
};
...
sjs.color = {
  ...
  at: function(x, y, d) {
    ...
    if (d.colorSamplers.length) {
      ...
      cs = sjs.color.addSamplers(x,y,d);
    }
    cs.move([new UnitValue(x, 'px' ),new UnitValue(y, 'px' )]);
    return cs.color;
  },
  // discussion:  http://ps-scripts.com/bb/viewtopic.php?f=2&t=2776 & http://ps-scripts.com/bb/viewtopic.php?t=2520
  picker: function() {
    var defaultColor = (app.foregroundColor.rgb.red << 16) + (app.foregroundColor.rgb.green << 8) + app.foregroundColor.rgb.blue,
        picked = $.colorPicker(defaultColor);
    $.writeln('picked: '+picked);
    if (-1 != picked) {
      var c = new SolidColor();
      c.rgb.red = (picked >> 16);
      c.rgb.green = (picked >> 8) & 0xFF;
      c.rgb.blue = (picked) & 0xFF;
      return c;
    } else {
      return null;
    }
...
sjs.random = {
  ...
  rgb: function(r0,r1,g0,g1,b0,b1) {
    var r = sjs.random.num(r0,r1);
    var g = sjs.random.num(g0,g1);
    var b = sjs.random.num(b0,b1);
